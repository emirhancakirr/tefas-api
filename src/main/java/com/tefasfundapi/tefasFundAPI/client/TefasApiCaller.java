...
public class TefasApiCaller {
    private static final Logger log = LoggerFactory.getLogger(TefasApiCaller.class);
     ...
     * 
     * @param body    : Sniffer'dan gördüğün body'yi buraya koy (JSON ise aynen,
     *                form-encoded ise ona göre).
     * @param headers : Gerekli ekstra header'lar (örn. RequestVerificationToken,
     *                X-Requested-With vs.)
     */
    public String postHistoryInfo(String body, Map<String, String> headers) {
        try (Playwright pw = Playwright.create()) {
            ...
            if (status == 401 || status == 403) {
                // cookie eskimiş olabilir → burada Session refresh akışını tetikleyip
                // (Sniffer/SessionManager),
